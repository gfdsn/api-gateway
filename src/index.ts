...
import { InvalidRequestException, MissingAuthenticationTokenException } from './requests/exceptions/request-exceptions.js';
...
const server: Server = http.createServer((req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
        ...
        RequestValidator.validateHeaders(headers, url);

        const request: Request = RequestHandler.parse(req);

        const service: string = request.service

        if (process.env.DEBUG) console.log(method + " " + request.target.getBaseUrl())
        ...
        } else if (err instanceof MissingAuthenticationTokenException) {

            res.writeHead(500, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({error: "The authentication token is missing."}));
            return;
